...
public class AdminDashboard extends ViewBase {
    ...
    public AdminDashboard(LoginResponse session) {
    ...
    private VBox buildSidebar(BorderPane root) {
        ...
        Label l2 = new Label("Адміністратар");
        ...
        bAllRooms.setOnAction(e -> {
            activateBtn(bAllRooms, all);
            root.setCenter(scrollWrap(allRoomsView()));
        });
        bAvailRooms.setOnAction(e -> {
            activateBtn(bAvailRooms, all);
            root.setCenter(scrollWrap(availRoomsView()));
        });
        bAddRoom.setOnAction(e -> {
            activateBtn(bAddRoom, all);
            root.setCenter(scrollWrap(addRoomView()));
        });
        bDelRoom.setOnAction(e -> {
            activateBtn(bDelRoom, all);
            root.setCenter(scrollWrap(deleteRoomView()));
        });
        bCloseRoom.setOnAction(e -> {
            activateBtn(bCloseRoom, all);
            root.setCenter(scrollWrap(closeRoomView()));
        });
        bOpenRoom.setOnAction(e -> {
            activateBtn(bOpenRoom, all);
            root.setCenter(scrollWrap(openRoomView()));
        });
        bAllRes.setOnAction(e -> {
            activateBtn(bAllRes, all);
            root.setCenter(scrollWrap(resTableView("Усе браніраванні", Operation.GET_ALL_RESERVATIONS)));
        });
        bPending.setOnAction(e -> {
            activateBtn(bPending, all);
            root.setCenter(scrollWrap(resTableView("Чакаюць зацверджання", Operation.GET_PENDING_RESERVATIONS)));
        });
        bApprove.setOnAction(e -> {
            activateBtn(bApprove, all);
            root.setCenter(scrollWrap(approveResView()));
        });
        bCancel.setOnAction(e -> {
            activateBtn(bCancel, all);
            root.setCenter(scrollWrap(cancelResView()));
        });
        bApproved.setOnAction(e -> {
            activateBtn(bApproved, all);
            root.setCenter(scrollWrap(resTableView("Зацверджаныя", Operation.GET_APPROVED_RESERVATIONS)));
        });
        bCheckOut.setOnAction(e -> {
            activateBtn(bCheckOut, all);
            root.setCenter(scrollWrap(checkOutView()));
        });
        bGuests.setOnAction(e -> {
            activateBtn(bGuests, all);
            root.setCenter(scrollWrap(guestTextView("Усе госці", Operation.GET_ALL_GUESTS)));
        });
        bGuestsRes.setOnAction(e -> {
            activateBtn(bGuestsRes, all);
            root.setCenter(scrollWrap(guestTextView("Госці з браніраваннямі", Operation.GET_ALL_GUESTS_WITH_RESERVATIONS)));
        });
        bEmployees.setOnAction(e -> {
            activateBtn(bEmployees, all);
            root.setCenter(scrollWrap(employeesView()));
        });
        bHire.setOnAction(e -> {
            activateBtn(bHire, all);
            root.setCenter(scrollWrap(hireView()));
        });
        bFire.setOnAction(e -> {
            activateBtn(bFire, all);
            root.setCenter(scrollWrap(fireView()));
        });
        bRole.setOnAction(e -> {
            activateBtn(bRole, all);
            root.setCenter(scrollWrap(changeRoleView()));
        });
        bProfile.setOnAction(e -> {
            activateBtn(bProfile, all);
    ...
    private VBox allRoomsView() {
        return roomTableView("Усе нумары", Operation.GET_ALL_ROOMS);
    }

    private VBox availRoomsView() {
        return roomTableView("Свабодныя нумары", Operation.GET_AVAILABLE_ROOMS);
    }
    ...
    private VBox addRoomView() {
        ...
        TextField priceFld = field("Кошт за ноч (BYN)");
        Button btn = new Button("Дадаць нумар");
        btn.getStyleClass().add("hotel-btn-primary");

        btn.setOnAction(e -> {
            int num, floor;
            float price;
            try {
                num = Integer.parseInt(numFld.getText().trim());
            } catch (NumberFormatException ex) {
                showError(msg, "Нумар — цэлы лік");
                return;
            }
            try {
                floor = Integer.parseInt(floorFld.getText().trim());
            } catch (NumberFormatException ex) {
                showError(msg, "Паверх — цэлы лік");
                return;
            }
            if (typeCombo.getValue() == null) {
                showError(msg, "Выберыце тып");
                return;
            }
            if (capCombo.getValue() == null) {
                showError(msg, "Выберыце месткасць");
                return;
            }
            try {
                price = Float.parseFloat(priceFld.getText().trim().replace(",", "."));
            } catch (NumberFormatException ex) {
                showError(msg, "Кошт — лік");
                return;
            }
            if (price <= 0) {
                showError(msg, "Кошт павінен быць станоўчай");
                return;
            }
            try {
        ...
        grid.addRow(2, vf("Кошт (BYN)", priceFld), new VBox());
    ...
    private VBox deleteRoomView() {
        ...
        Button btn = new Button("Выдаліць нумар");
        btn.getStyleClass().add("hotel-btn-danger");
        btn.setDisable(true);
        combo.getSelectionModel().selectedItemProperty().addListener((o, old, sel) -> btn.setDisable(sel == null));
        btn.setOnAction(e -> {
            Room sel = combo.getValue();
            if (sel == null) return;
            if (!confirm("Выдаліць", "Выдаліць нумар #" + sel.getNumber() + "? Нельга скасаваць.")) return;
            try {
                Response resp = send(new Request(Operation.DELETE_ROOM, mapper.writeValueAsString(sel.getNumber())));
                if (resp != null && resp.isSuccess()) {
                    showSuccess(msg, resp.getMessage());
                    reloadAllRooms(combo, msg);
                } else showError(msg, resp != null ? resp.getMessage() : "Памылка");
            } catch (Exception ex) {
                showError(msg, ex.getMessage());
            }
        });
        VBox form = new VBox(10, fieldLabel("Нумар пакоя"), combo, msg, btn);
        form.setMaxWidth(440);
        box.getChildren().add(card("Выдаліць нумар", form));
        return box;
    }

    private VBox closeRoomView() {
        return roomActionView("Зачыніць нумар", "Зачыніць", Room.Status.AVAILABLE, Operation.CLOSE_ROOM, false);
    }

    private VBox openRoomView() {
        return roomActionView("Адчыніць нумар", "Адчыніць", Room.Status.MAINTENANCE, Operation.OPEN_ROOM, true);
    }
    ...
    private VBox roomActionView(String title, String btnText, Room.Status filterStatus, Operation op, boolean success) {
        ...
        btn.getStyleClass().add(success ? "hotel-btn-success" : "hotel-btn-danger");
        btn.setDisable(true);
        combo.getSelectionModel().selectedItemProperty().addListener((o, old, sel) -> btn.setDisable(sel == null));
        btn.setOnAction(e -> {
            Room sel = combo.getValue();
            if (sel == null) return;
            try {
                Response resp = send(new Request(op, mapper.writeValueAsString(sel.getNumber())));
                if (resp != null && resp.isSuccess()) {
                    showSuccess(msg, resp.getMessage());
                    reloadByStatus(combo, msg, filterStatus);
                } else showError(msg, resp != null ? resp.getMessage() : "Памылка");
            } catch (Exception ex) {
                showError(msg, ex.getMessage());
            }
        ...
        VBox form = new VBox(10, fieldLabel("Нумар пакоя"), combo, msg, btn);
        form.setMaxWidth(440);
        box.getChildren().add(card(title, form));
        return box;
    }
    ...
    private VBox resTableView(String title, Operation op) {
        ...
        box.getChildren().add(card(title, new HBox(8, refresh), table, msg));
        loadResToTable(table, msg, op);
        return box;
    }

    private VBox approveResView() {
        return resActionView("Зацвердзіць браніраванне", "Зацвердзіць", true, Operation.GET_PENDING_RESERVATIONS, Operation.APPROVE_RESERVATION);
    }

    private VBox cancelResView() {
        return resActionView("Скасаваць браніраванне", "Скасаваць", false, Operation.GET_PENDING_RESERVATIONS, Operation.CANCEL_RESERVATION);
    }
    ...
    private VBox resActionView(String title, String btnText, boolean isSuccess, Operation loadOp, Operation actionOp) {
        ...
        Button btn = new Button(btnText);
        btn.getStyleClass().add(isSuccess ? "hotel-btn-success" : "hotel-btn-danger");
        btn.setDisable(true);
        combo.getSelectionModel().selectedItemProperty().addListener((o, old, sel) -> btn.setDisable(sel == null));
        ...
        btn.setOnAction(e -> {
            ...
            try {
                Response resp = send(new Request(actionOp, mapper.writeValueAsString(sel.getId())));
                if (resp != null && resp.isSuccess()) {
                    showSuccess(msg, resp.getMessage());
                    reloadResCombo(combo, msg, loadOp);
                } else showError(msg, resp != null ? resp.getMessage() : "Памылка");
            } catch (Exception ex) {
                showError(msg, ex.getMessage());
            }
        });
        VBox form = new VBox(12, fieldLabel("Браніраванне"), combo, new HBox(8, refresh), msg, btn);
    ...
    private VBox checkOutView() {
        ...
        Button btn = new Button("Выселіць і выставіць адзнаку");
        btn.getStyleClass().add("hotel-btn-primary");
        btn.setDisable(true);
        combo.getSelectionModel().selectedItemProperty().addListener((o, old, sel) -> btn.setDisable(sel == null));
        ...
        btn.setOnAction(e -> {
            ...
            try {
                ...
                if (resp != null && resp.isSuccess()) {
                    showSuccess(msg, resp.getMessage());
                    combo.setValue(null);
                    reloadResCombo(combo, msg, Operation.GET_APPROVED_RESERVATIONS);
                } else showError(msg, resp != null ? resp.getMessage() : "Памылка");
            } catch (Exception ex) {
                showError(msg, ex.getMessage());
            }
        });
        VBox form = new VBox(12, fieldLabel("Зацверджанае браніраванне"), combo, new HBox(8, refresh), msg, btn);
    ...
    private VBox employeesView() {
        VBox box = contentBox();
        TableView<Employee> table = TableHelper.employeeTable();
        table.setPrefHeight(480);
        Label msg = errorLabel();
        Button refresh = refreshBtn(() -> loadEmps(table, msg));
        box.getChildren().add(card("Спіс супрацоўнікаў", new HBox(8, refresh), table, msg));
        loadEmps(table, msg);
        return box;
    }
    ...
    private VBox hireView() {
        ...
        Button btn = new Button("Уладкаваць");
        btn.getStyleClass().add("hotel-btn-primary");
        btn.setOnAction(e -> {
            int id;
            float sal;
            LocalDate date;
            try {
                id = Integer.parseInt(accId.getText().trim());
            } catch (NumberFormatException ex) {
                showError(msg, "Няправільны ID");
                return;
            }
            if (posCombo.getValue() == null) {
                showError(msg, "Выберыце пасаду");
                return;
            }
            try {
                sal = Float.parseFloat(salary.getText().trim().replace(",", "."));
            } catch (NumberFormatException ex) {
                showError(msg, "Заробак — лік");
                return;
            }
            if (sal <= 0) {
                showError(msg, "Заробак павінен быць станоўчым");
                return;
            }
            try {
                date = LocalDate.parse(hireDate.getText().trim());
            } catch (DateTimeParseException ex) {
                showError(msg, "Фармат: гггг-мм-дд");
                return;
            }
            try {
                Employee emp = new Employee(id, posCombo.getValue(), sal, date);
        ...
        box.getChildren().add(card("Уладкаваць супрацоўніка", grid, msg, btn));
        return box;
    }
    ...
    private VBox fireView() {
        VBox box = contentBox();
        Label msg = errorLabel();
        TableView<Employee> empTable = TableHelper.employeeTable();
        empTable.setPrefHeight(220);
        loadEmps(empTable, errorLabel());
        TextField accId = field("ID акаўнта для звальнення");
        Button btn = new Button("Звольніць");
        btn.getStyleClass().add("hotel-btn-danger");
        btn.setOnAction(e -> {
            int id;
            try {
            ...
            try {
                Response resp = send(new Request(Operation.FIRE_EMPLOYEE, mapper.writeValueAsString(id)));
                if (resp != null && resp.isSuccess()) {
                    showSuccess(msg, resp.getMessage());
                    accId.clear();
                    loadEmps(empTable, errorLabel());
        ...
        VBox form = new VBox(12, fieldLabel("Бягучыя супрацоўнікі"), empTable, fieldLabel("ID акаўнта"), accId, msg, btn);
        form.setMaxWidth(660);
        box.getChildren().add(card("Звольніць супрацоўніка", form));
        return box;
    }
    ...
    private VBox changeRoleView() {
        ...
        TextField accId = field("ID акаўнта");
        ComboBox<Employee.Position> posCombo = posCombo();
        Button btn = new Button("Змяніць пасаду");
        btn.getStyleClass().add("hotel-btn-primary");
        btn.setOnAction(e -> {
            int id;
            try {
            ...
            if (posCombo.getValue() == null) {
                showError(msg, "Выберыце новую пасаду");
                return;
            }
            try {
                ChangeRoleRequest req = new ChangeRoleRequest(id, posCombo.getValue());
        ...
        VBox form = new VBox(10, fieldLabel("ID акаўнта"), accId, fieldLabel("Новая пасада"), posCombo, msg, btn);
        form.setMaxWidth(420);
        box.getChildren().add(card("Змяніць пасаду", form));
        return box;
    }
    ...
    private VBox profileView() {
        ...
        VBox form = new VBox(10, fieldLabel("Email"), emailFld, fieldLabel("Імя"), firstFld, fieldLabel("Прозвішча"), lastFld, fieldLabel("Пароль"), passFld, errLbl, saveBtn);
        form.setMaxWidth(420);
    ...
    private void loadRoomsToTable(TableView<Room> t, Label msg, Operation op) {
        try {
            Response resp = send(new Request(op, null));
            if (resp != null && resp.isSuccess())
                t.setItems(FXCollections.observableArrayList(mapper.<List<Room>>readValue(resp.getData(), new TypeReference<>() {
                })));
            else showError(msg, resp != null ? resp.getMessage() : "Памылка");
        } catch (Exception ex) {
            showError(msg, ex.getMessage());
    ...
    private void loadResToTable(TableView<Reservation> t, Label msg, Operation op) {
        try {
            Response resp = send(new Request(op, null));
            if (resp != null && resp.isSuccess())
                t.setItems(FXCollections.observableArrayList(mapper.<List<Reservation>>readValue(resp.getData(), new TypeReference<>() {
                })));
            else showError(msg, resp != null ? resp.getMessage() : "Памылка");
        } catch (Exception ex) {
            showError(msg, ex.getMessage());
    ...
    private void loadEmps(TableView<Employee> table, Label msg) {
        try {
            Response resp = send(new Request(Operation.GET_ALL_EMPLOYEES, null));
            if (resp != null && resp.isSuccess()) {
                List<Employee> list = mapper.readValue(resp.getData(), new TypeReference<>() {
                });
                table.setItems(FXCollections.observableArrayList(list));
                if (msg != null) clearMsg(msg);
            } else if (msg != null) showError(msg, resp != null ? resp.getMessage() : "Памылка");
        } catch (Exception ex) {
            if (msg != null) showError(msg, ex.getMessage());
        }
    }
    ...
    private void reloadAllRooms(ComboBox<Room> combo, Label msg) {
        try {
            ...
            if (resp != null && resp.isSuccess())
                combo.setItems(FXCollections.observableArrayList(mapper.<List<Room>>readValue(resp.getData(), new TypeReference<>() {
                })));
            else showError(msg, resp != null ? resp.getMessage() : "Памылка");
        } catch (Exception ex) {
            showError(msg, ex.getMessage());
    ...
    private ComboBox<Room> roomsByStatus(Label msg, Room.Status status) {
        ...
        combo.setCellFactory(lv -> roomCell());
        combo.setButtonCell(roomCell());
        reloadByStatus(combo, msg, status);
        return combo;
    }
    ...
    private void reloadResCombo(ComboBox<Reservation> combo, Label msg, Operation op) {
        try {
            Response resp = send(new Request(op, null));
            if (resp != null && resp.isSuccess())
                combo.setItems(FXCollections.observableArrayList(mapper.<List<Reservation>>readValue(resp.getData(), new TypeReference<>() {
                })));
            else showError(msg, resp != null ? resp.getMessage() : "Памылка");
        } catch (Exception ex) {
            showError(msg, ex.getMessage());
    ...
    private ComboBox<Room.Type> typeCombo() {
        ComboBox<Room.Type> c = new ComboBox<>();
        c.getStyleClass().add("hotel-combo");
        c.setMaxWidth(Double.MAX_VALUE);
        c.getItems().addAll(Room.Type.values());
        c.setCellFactory(lv -> new ListCell<>() {
            @Override
            protected void updateItem(Room.Type t, boolean e) {
                super.updateItem(t, e);
                setText(e || t == null ? "" : TableHelper.roomTypeStr(t));
            }
        });
        c.setButtonCell(new ListCell<>() {
            ...
            protected void updateItem(Room.Type t, boolean e) {
                super.updateItem(t, e);
                setText(e || t == null ? "Тып" : TableHelper.roomTypeStr(t));
            }
        });
        return c;
    }
    ...
    private ComboBox<Room.Capacity> capCombo() {
        ComboBox<Room.Capacity> c = new ComboBox<>();
        c.getStyleClass().add("hotel-combo");
        c.setMaxWidth(Double.MAX_VALUE);
        c.getItems().addAll(Room.Capacity.values());
        c.setCellFactory(lv -> new ListCell<>() {
            @Override
            protected void updateItem(Room.Capacity cap, boolean e) {
                super.updateItem(cap, e);
                setText(e || cap == null ? "" : TableHelper.capacityStr(cap));
            }
        });
        c.setButtonCell(new ListCell<>() {
            @Override
            protected void updateItem(Room.Capacity cap, boolean e) {
                super.updateItem(cap, e);
                setText(e || cap == null ? "Месткасць" : TableHelper.capacityStr(cap));
            }
        });
        return c;
    }
    ...
    private ComboBox<Employee.Position> posCombo() {
        ComboBox<Employee.Position> c = new ComboBox<>();
        c.getStyleClass().add("hotel-combo");
        c.setMaxWidth(Double.MAX_VALUE);
        c.getItems().addAll(Employee.Position.values());
        c.setCellFactory(lv -> new ListCell<>() {
            @Override
            protected void updateItem(Employee.Position p, boolean e) {
                super.updateItem(p, e);
                setText(e || p == null ? "" : posToStr(p));
            }
        });
        c.setButtonCell(new ListCell<>() {
            @Override
            protected void updateItem(Employee.Position p, boolean e) {
                super.updateItem(p, e);
                setText(e || p == null ? "Пасада" : posToStr(p));
            }
        });
        return c;
    }
    ...
    private ListCell<Room> roomCell() {
        return new ListCell<>() {
            @Override
            protected void updateItem(Room r, boolean e) {
                super.updateItem(r, e);
                setText(e || r == null ? "" : "#" + r.getNumber() + " — " + TableHelper.roomTypeStr(r.getType()) + " | " + TableHelper.roomStatusStr(r.getStatus()));
            }
        };
    }
    ...
    private ListCell<Reservation> resCell() {
        return new ListCell<>() {
            @Override
            protected void updateItem(Reservation r, boolean e) {
                super.updateItem(r, e);
                setText(e || r == null ? "" : "#" + r.getId() + " | Нумар " + r.getRoomNumber() + " | Госць " + r.getGuestId() + " | " + r.getReservationDate() + " \u00d7 " + r.getDuration() + " ноч.");
            }
        };
    }

    private VBox vf(String label, javafx.scene.Node control) {
        return new VBox(6, fieldLabel(label), control);
    }
}
